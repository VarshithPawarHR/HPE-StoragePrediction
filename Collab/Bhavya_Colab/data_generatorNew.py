...
class StorageSimulator:
    def __init__(self, mount_points_config):
        ...
        self.size_check_interval = 60  # Check directory sizes every minute (reduced from 5 minutes)
    ...
    def get_random_size_with_delay(self, mount_config):
        ...
        sizes = [
            # Format: (min_size, max_size, probability, (min_delay, max_delay))
            (1024 * 10, 1024 * 500, 0.4, (5, 10)),         # 10KB - 500KB: 5-10 seconds
            (1024 * 500, 1024 * 1024 * 5, 0.3, (10, 20)),  # 500KB - 5MB: 10-20 seconds
            (1024 * 1024 * 5, 1024 * 1024 * 100, 0.2, (20, 40)),  # 5MB - 100MB: 20-40 seconds
            (1024 * 1024 * 100, 1024 * 1024 * 1024, 0.1, (30, 60))  # 100MB - 1GB: 30-60 seconds
        ]
        ...
        return random.randint(1024 * 100, 1024 * 1024), (5, 15)
    ...
    def create_file(self, mount_point, mount_config):
        ...
        try:
            ...
            # Update directory size after creation - asynchronously to avoid slowing down creation
            threading.Thread(target=self.update_directory_size, args=(mount_point,), daemon=True).start()
    ...
    def update_file(self, filepath):
        ...
        try:
            ...
            # Update directory size after modification - async
    ...
    def delete_file(self, filepath):
        ...
        try:
            if os.path.exists(filepath) and not self.is_protected_path(filepath):
                ...
                # Update directory size after deletion - async
                mount_point = os.path.dirname(filepath)
                threading.Thread(target=self.update_directory_size, args=(mount_point,), daemon=True).start()
                return True
            return False
    ...
    def simulate_mount_point(self, mount_config):
        ...
        def worker():
            while self.running:
                try:
                    ...
                    delay_range = (5, 10)  # default delay - much faster
                    ...
                    if operation == 'create' and not is_full:
                        ...
                        # If we couldn't perform any operation (e.g., no files to update/delete)
                        # Try to create a file instead
                        if not is_full:
                            filepath, delay_range = self.create_file(mount_point, mount_config)
                    
                    # Random sleep with exponential backoff if we have many files
                    jitter = random.uniform(0.8, 1.2)  # Add some randomness
                    delay = random.uniform(*delay_range) * jitter
                    
                    # Shorter delay when fewer files exist to build up quickly
                    if num_files < 100 and operation == 'create':
                        delay = delay * 0.5
        ...
        # Increase worker count for more concurrent operations
    ...
    def start(self):
        ...
        with ThreadPoolExecutor(max_workers=len(self.mount_points_config) * 2) as executor:
            for mount_config in self.mount_points_config:
...
if __name__ == "__main__":
    mount_points_config = [
        {
            ...
            'operation_weights': {'create': 0.7, 'update': 0.2, 'delete': 0.1},  # Higher creation weight
            'size_weights': {
                '10240-512000': 1.5,  # Favor smaller files for faster creation
                '512000-5242880': 1.0,
                '5242880-104857600': 0.7,
                '104857600-1073741824': 0.4
            },
            'update_delay_min': 15,  # 15 seconds
            'update_delay_max': 30,   # 30 seconds
            'delete_delay_min': 30,   # Keep deletion rate similar to original
            'delete_delay_max': 60
        },
        
        {
            'path': '/scratch',
            'num_workers': 12,  # Increased from 3
            'size_cap': 2.5 * 1024 * 1024 * 1024 * 1024,  # 2.5TB
            'operation_weights': {'create': 0.8, 'update': 0.1, 'delete': 0.1},  # Heavy focus on creation
            'size_weights': {
                '10240-512000': 1.2,
                '512000-5242880': 1.3,  # Favor medium files
                '5242880-104857600': 0.8,
                '104857600-1073741824': 0.5
            },
            'update_delay_min': 10,  # Reduced from 120
            'update_delay_max': 20,   # Reduced from 180
            'delete_delay_min': 120,  # Keep deletion rate similar
            'delete_delay_max': 180
        },
        {
            'path': '/projects',
            'num_workers': 10,  # Increased from 2
            'size_cap': 2.5 * 1024 * 1024 * 1024 * 1024,  # 2.5TB
            'operation_weights': {'create': 0.75, 'update': 0.15, 'delete': 0.1},  # Favor creation
            'size_weights': {
                '10240-512000': 0.6,
                '512000-5242880': 0.8,
            ...
            'update_delay_min': 20,  # Reduced from 240
            'update_delay_max': 40,   # Reduced from 240
            'delete_delay_min': 240,  # Keep deletion rate similar
        ...
        },
        {
            ...
            'num_workers': 8,  # Increased from 2
            'size_cap': 2.5 * 1024 * 1024 * 1024 * 1024,  # 2.5TB
            'operation_weights': {'create': 0.8, 'update': 0.1, 'delete': 0.1},  # Heavily favor creation
            'size_weights': {
                '10240-512000': 0.5,
                '512000-5242880': 0.7,
            ...
            'update_delay_min': 10,  # Reduced from 60
            'update_delay_max': 20,   # Reduced from 60
            'delete_delay_min': 60,   # Keep deletion rate similar
            'delete_delay_max': 60
        }
    ]
    ...
    try:
        print("Starting high-speed storage simulator...")
        print("\nDirectory size caps:")
        for config in mount_points_config:
            print(f"{config['path']}: {config['size_cap'] / (1024**4):.1f}TB")
        
        print("\nFile generation frequencies (significantly accelerated):")
        print("/customers: 5-10 seconds with 8 workers")
        print("/scratch: 10-20 seconds with 12 workers")
        print("/projects: 20-40 seconds with 10 workers")
        print("/info: 10-20 seconds with 8 workers")
        
        print("\nDeletion frequencies (kept similar to original):")
        print("/customers: 30-60 seconds")
        print("/scratch: 120-180 seconds")
        print("/projects: 240 seconds")
        print("/info: 60 seconds")
        
        simulator.start()
        
        # Main loop with size monitoring display
        while True:
            time.sleep(30)  # Update display every 30 seconds (more frequent than original)
